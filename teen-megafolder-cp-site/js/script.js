    ...
    popup.innerHTML = `
        <div class="popup-content">
            ...
            <div class="popup-buttons">
                <a href="https://buy.stripe.com/dRmcN62si6pf43Y9OL8EM0U" class="popup-button" target="_blank" rel="noopener">
                    <i class="fas fa-lock"></i> GET DISCOUNT NOW
                </a>
                <a href="https://t.me/PRO_sellerhere?text=Hello!%20I%20would%20like%20to%20buy%0A%0AContent:%20Access%20to%20the%20VIP%20group%20of%20Cp,%20Teens%20and%20all%20other%20leaked%20content%0ADescription:%0AExclusive%20high%20quality%20content%20%E2%80%A2%20Lifetime%20access%20%E2%80%A2%20Instant%20delivery%0ABonus:%20Two%20folders%20of%20the%20mega%20link%20of%20cp%0A%0APrice:%20$%2035" 
...
document.addEventListener('click', function(event) {
  ...
  const exitPopupContent = document.querySelector('.exit-popup .popup-content');
  
  if (exitPopup && exitPopupContent) {
    // Verifica se o clique foi fora do conteúdo do popup
    if (exitPopup.contains(event.target) && !exitPopupContent.contains(event.target)) {
      exitPopup.style.display = 'none';
    }
  }
  
  // Para o popup de fim de vídeo
  const endPopup = document.querySelector('.end-popup');
  const endPopupContent = document.querySelector('.end-popup .popup-content');
  
  if (endPopup && endPopupContent) {
    // Verifica se o clique foi fora do conteúdo do popup
    if (endPopup.contains(event.target) && !endPopupContent.contains(event.target)) {
      endPopup.style.display = 'none';
    }
  }
});

// Também fechar o popup quando o botão X for clicado
document.addEventListener('DOMContentLoaded', function() {
  // Para o botão de fechar no popup de saída
  const closeExitPopupButton = document.querySelector('.exit-popup .close-popup');
  if (closeExitPopupButton) {
    closeExitPopupButton.addEventListener('click', function() {
      const exitPopup = document.querySelector('.exit-popup');
      if (exitPopup) {
        exitPopup.style.display = 'none';
      }
  ...
  // Para o botão de fechar no popup de fim de vídeo
  const closeEndPopupButton = document.querySelector('.end-popup .close-popup');
  if (closeEndPopupButton) {
    closeEndPopupButton.addEventListener('click', function() {
      const endPopup = document.querySelector('.end-popup');
      if (endPopup) {
        endPopup.style.display = 'none';
      }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Função para atualizar o ícone de mute
    ...
    // Atualizar ícones quando o vídeo mudar de estado
